...
These environment variables only affect build-time tools. The packaged app's
runtime download already uses the Chromium network stack, which trusts the OS
certificate store and reads system proxy settings, so the app itself does not
require per-user `NODE_OPTIONS`.

API keys are entered in the app's **Settings → Models** and stored under the
user home directory (`~/.dsh`); the packaged app does not read `.env` from the
repository.

## Icons

The application icon features the **DeepSeek Whale Girl** character, originally
created by [fornarwhal](https://github.com/fornarwhal). The assets are sourced
from the
[fornarwhal/deepseek-whale-girl-icon](https://github.com/fornarwhal/deepseek-whale-girl-icon)
repository. The master source image is `improved-1.png` (984×984, RGBA), scaled
to each platform's required sizes:

| File | Source | Purpose |
|---|---|---|
| `build/icon.ico` | `DeepSeekHarness-WhaleGirl.ico` (16/24/32/48/64/128/256) | Windows app, taskbar, shortcuts |
| `build/icon.icns` | Assembled from `improved-1.png` (16–1024) | macOS app |
| `build/icon.png` | Scaled from `improved-1.png` (256×256) | Linux AppImage + fallback |
| `resources/trayTemplate.png` | Scaled from `improved-1.png` (32×32) | tray (macOS template) |
| `resources/tray.png` | Scaled from `improved-1.png` (32×32) | tray |

To refresh the icons, regenerate the PNGs from the master source and reassemble
