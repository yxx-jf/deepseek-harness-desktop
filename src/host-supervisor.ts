...
export interface ReadinessParser {
  /** Consume one stdout chunk; returns the loopback URL once a complete readiness line is observed. */
  ...
  const start = (): Promise<string> => {
    ...
    startPromise = new Promise<string>((resolve, reject) => {
      ...
      const acceptChunk = (chunk: string): void => {
        // Forward every chunk to the diagnostic log; the readiness parser
        // consumes only until the first ready line, so later look-alike output
        // cannot throw a conflicting-URL error.
        appendOutput(chunk)
        if (settled) return
        try {
          ...
          settled = true
          ready = true
          // Stop the readiness timer but keep the stdout/stderr listeners:
          // Host warnings and failures stay visible after startup.
          clearTimeout(timer)
          resolve(url)
        } catch (error) {
          fail(error)
