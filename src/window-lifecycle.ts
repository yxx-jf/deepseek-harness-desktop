...
export interface DesktopLifecycle {
  ...
  /**
   * Hide an ordinary close, quit when a tray-hidden window is closed
   * externally, or let closes proceed during explicit quit.
   */
...
export function createDesktopLifecycle(options: DesktopLifecycleOptions): DesktopLifecycle {
  ...
  return {
    ...
    onWindowClose(event) {
      ...
      // A tray-hidden window cannot be closed by the user (no close button is
      // visible), so a close arriving while the window is hidden is external —
      // the NSIS installer/uninstaller graceful close, a Windows session end,
      // or shutdown — and must quit the application instead of re-hiding it.
      // Quitting from inside the process also succeeds where the installer's
      // force-kill would be denied (an elevated app killed by an unelevated
      // installer), because no external kill rights are needed.
