  ...
  vi.doMock('koffi', () => ({
    default: {
      ...
      decode: Object.assign(
        ...
        // The string16 value decoder reads the NUL-terminated UTF-16 text at
        // the address directly (no external Buffer, no pointer dereference).
        { string16: (pointer: unknown): string => (pointer as FakePtr).text as string ?? '' },
      ),
