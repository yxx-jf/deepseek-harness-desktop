...
应用图标使用了 **DeepSeek Whale Girl**（深海鲸娘）形象，原作者为
[fornarwhal](https://github.com/fornarwhal)。素材来源于
[fornarwhal/deepseek-whale-girl-icon](https://github.com/fornarwhal/deepseek-whale-girl-icon)
仓库。主源图为 `improved-1.png`（984×984，RGBA），缩放至各平台所需尺寸：

| 文件 | 来源 | 用途 |
|---|---|---|
| `build/icon.ico` | `DeepSeekHarness-WhaleGirl.ico` (16/24/32/48/64/128/256) | Windows 应用、任务栏、快捷方式 |
| `build/icon.icns` | 从 `improved-1.png` 组装 (16–1024) | macOS 应用 |
| `build/icon.png` | 从 `improved-1.png` 缩放 (256×256) | Linux AppImage + 回退 |
| `resources/trayTemplate.png` | 从 `improved-1.png` 缩放 (32×32) | 托盘图标（macOS 模板） |
| `resources/tray.png` | 从 `improved-1.png` 缩放 (32×32) | 托盘图标 |

要刷新图标，从主源图重新生成 PNG 并重新组装 `icon.ico` 和 `icon.icns`。
