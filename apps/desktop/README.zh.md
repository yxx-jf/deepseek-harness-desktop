...
应用携带设计师产出的多平台图标集（来自 `DeepSeek Harness_icons` LogoGen 导出）。各尺寸 PNG 被**逐字节**包进容器格式——精确像素原样嵌入，不做缩放或重编码——并作为静态资源提交：
...
要从新的设计师导出刷新，就把 PNG 重新包进 `icon.ico` / `icon.icns`（与导出所用的容器装配相同）并重新打包。
